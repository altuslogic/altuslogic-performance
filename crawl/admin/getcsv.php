<?php
   ...
   $DbDatabase = "crawl_communedeparisxfr"; // The database you are going to use
   ...
   */
    ...
    */ 
      ...
      function  numb_count_center($col,$cols_sel,$item_sel,$publish){  
           ...
           function  showitems_url($cols_sel,$item_sel){ 
                                 /*   $sel_select="AND `".$cols_sel."`='".$item_sel."'"; 
                                    $sel_select2="`$cols_sel`='$item_sel' AND"; 
                                    $sel_select3="WHERE `$cols_sel`='$item_sel'"; 
                                  ...
                                  */  
                                    ...
                                    $result=mysql_query("SELECT * FROM `links` WHERE `final_url` IS NOT NULL GROUP BY `final_url`");
                                    $aa.= "<br><b>$item_sel</b> ($as / $ps)<br> "; 
                         
                                   while($row=mysql_fetch_array($result)){ 
                                            $aa.= $row[final_url]."~".$row[url]."~".$row[final_detail1]."~<br>";
                                   }
      ...
      function  showitems($cols_sel,$item_sel){ 
                             ...
                             */  
                               // WHERE $sel_select2 WHERE $sel_select2 WHERE $sel_select2
                               $result=mysql_query("SELECT * FROM `links` WHERE `final_email` IS NOT NULL ORDER BY `final_url` DESC LIMIT 50000");
                               $aa.= "<br><b>$item_sel</b> ($as / $ps)<br> "; 
                    
                              while($row=mysql_fetch_array($result)){ 
                                  
                                     $name = substr($row[final_titre], 0, 40); 
									 ...
									 $email = str_replace(';', '', $email);
									 $email = str_replace("'", '', $email);
									 $contact = $row[final_contact];
									 $aa.= "".$name."~".$contact."~".strtolower($email)."~".$row[final_url]."~".FormatTel($row[final_telephone])."~".FormatTel($row[final_telecopie])."~".$row[final_adresse]."~".$row[final_detail2]."~".$row[final_detail5]."~".$row[final_detail1]."~<br>";
