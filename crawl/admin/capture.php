<?php
    ...
    function capture($url,$folder){
        ...
        if ($image_type == 1) $format = 'gif';
        if ($image_type == 2) $format = 'jpg';
        if ($image_type == 3) $format = 'png';

        $filename = $folder.cleanDir($url).'.'.$format;

        // See if we have cached website screenshot image (to minimize server load)
        if (!file_exists($filename) || filemtime ($filename) + $cache_expire_time*60 < time()){
        ...
        if (!file_exists($filename)){
            echo "Thumbnail Generation Error. Thumbnail not created.";
            return;
        }
        ...
        $img->generateThumbFile($filename, '');
